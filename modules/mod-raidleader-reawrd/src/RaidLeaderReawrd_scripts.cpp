...
std::vector<RaidLeaderReawrd> LeaderReawrdData;
...
public:
    ...
    /*void OnBeforeSetBossState(uint32 id, EncounterState newState, EncounterState oldState, Map* instance) override
    {
       ...
       LOG_ERROR("module", "BOSS:{}  newState{}", boss->GetName(), newState);
       if(!boss || boss->IsAlive()) {
          LOG_ERROR("module", "BOSS:{} ({}) 状态完成-但还活着",boss->GetName(),boss->GetEntry());
          return;
       }
        ...
        for (auto itr = LeaderReawrdData.begin(); itr != LeaderReawrdData.end(); ++itr)
        {
            ...
            for (Map::PlayerList::const_iterator i = playerList.begin(); i != playerList.end(); ++i)
            {
                ...
                if (Group* group = player->GetGroup()) {
                    ...
                    LOG_INFO("module", "团长奖励BOSS:{} ({})", (*itr).bossid, plcount);
                    break;
                }
            }
            break;
