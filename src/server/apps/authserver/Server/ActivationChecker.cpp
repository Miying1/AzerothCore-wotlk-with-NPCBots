...
void ActivationChecker::Initialize()
{
    ...
    {
        LOG_WARN("server.authserver", "[Activation] activate.key 格式无效（payload 或签名为空）");
        return;
    }
    ...
    try
    {
        ...
        {
            LOG_WARN("server.authserver", "[Activation] 签名验证失败 - 激活密钥无效");
            return;
        }
    }
    catch (std::exception const& e)
    {
        LOG_WARN("server.authserver", "[Activation] 签名验证异常: {}", e.what());
        return;
    }

    // 签名已验证；现在解析 payload: code:MachineGuid|motherboardUUID
    size_t colonPos = payload.find(':');
    if (colonPos == std::string::npos)
    {
        LOG_WARN("server.authserver", "[Activation] payload 格式无效（缺少冒号分隔符）");
        return;
    }
    ...
    {
        LOG_WARN("server.authserver", "[Activation] payload 格式无效（缺少管道分隔符）");
        return;
    }
    ...
    {
        _isActivated = true;
        LOG_INFO("server.authserver", "[Activation] 服务器激活成功 (code: {}, MachineGuid 匹配: {}, UUID 匹配: {})", code, guidMatch, uuidMatch);
    }
    else
    {
        LOG_WARN("server.authserver", "[Activation] 机器标识符不匹配 (key Guid: {}, local Guid: {}, key UUID: {}, local UUID: {})",
...
bool ActivationChecker::RSAVerifySignature(std::string const& keyPath, std::string const& payload, std::vector<uint8> const& signature)
{
    ...
    // 加载公钥（与 crypto.go 匹配: savePublicKey 使用 MarshalPKCS1PublicKey -> "RSA PUBLIC KEY"）
    ...
    // SHA256 哈希 payload（与 crypto.go 匹配: sha256.Sum256）
    unsigned char hash[SHA256_DIGEST_LENGTH];
    SHA256(reinterpret_cast<const unsigned char*>(payload.data()), payload.size(), hash);

    // 使用 RSA 公钥验证签名（PKCS1v15, SHA256 - 与 crypto.go 匹配: rsa.SignPKCS1v15 + crypto.SHA256）
    ...
    if (verifyResult != 1)
    {
        ...
        LOG_DEBUG("server.authserver", "[Activation] RSA 签名验证失败: {}", errBuf);
        return false;
    }
