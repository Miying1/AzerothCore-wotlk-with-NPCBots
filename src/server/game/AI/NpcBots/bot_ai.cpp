    ...
    if (!pGroup)
    {
        ...
        if (HasRole(BOT_ROLE_HEAL))
        {
            ...
            for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
            {
                ...
                if (!(!u || !u->IsAlive()|| !u->IsInWorld() || u->HasUnitState(UNIT_STATE_ISOLATED) || me->GetDistance(u) > 40 || GetHealthPCT(u) > hppctthreshold)) 
                    targets3.push_back(u);
...
bool bot_ai::OnGossipHello(Player* player, uint32 /*option*/)
{
    ...
    if (_ownerGuid)
    {
        ...
        if (player == master)
        {
            ...
            //LocalizedNpcText(player, BOT_TEXT_PULL_URSELF)
