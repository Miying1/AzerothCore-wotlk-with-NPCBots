...
// NPCBot 幻形用：某个 DisplayId 的"真实尺寸系数"
...
// 返回 0 表示该 DisplayId 缺几何数据，调用方退化为 BotTransmogDisplayScaleFactor()。
static float BotTransmogDisplaySizeFactor(uint32 displayId)
{
...
static float BotTransmogDisplayScaleFactor(uint32 displayId)
{
    CreatureDisplayInfoEntry const* info = sCreatureDisplayInfoStore.LookupEntry(displayId);
    if (!info)
        return 0.f;
...
bool Creature::CalculateBotTransmogScale(uint32 srcDisplayId, uint32 dstDisplayId, float srcDisplayScale, float& outScale)
{
    ...
    if (srcSize <= 0.f || dstSize <= 0.f)
    {
        ...
        srcSize = BotTransmogDisplayScaleFactor(srcDisplayId);
        dstSize = BotTransmogDisplayScaleFactor(dstDisplayId);
        if (srcSize <= 0.f)
            srcSize = 1.f;
        if (dstSize <= 0.f)
            return false;                       // 幻形模型数据异常，放弃计算
    }

    // 1) 高度归一：让幻形后的最终高度等于 BOT 原高度
    //    (原) = srcDisplayScale × 原模型尺寸系数
    //    (新) = scale           × 幻形模型尺寸系数
    float baseScale = srcDisplayScale * srcSize / dstSize;

    // 2) 目标模型远高于 BOT 原模型时，按高度对齐后视觉上会显小，补一点体量（加成只增不减，
    //    故最终缩放恒 >= baseScale，即幻形后的高度不会小于 BOT 原本的高度）：
    //    高出 100% 以上（超过 2 倍）→ 缩放 +20%；高出 50% 以上（超过 1.5 倍）→ 缩放 +10%；其余不加
...
{
    // NPCBot 幻形：原生显示 ID 已被改成幻形模型，返回高度归一后的 scale
    if (IsNPCBot())
    {
