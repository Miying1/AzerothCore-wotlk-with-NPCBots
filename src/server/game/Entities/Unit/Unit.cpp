...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown, uint32 procPhase, ProcEventInfo& eventInfo)
{
    ...
    if (!triggerEntry)
    {
        ...
        //LOG_DEBUG("entities.unit", "Unit::HandleProcTriggerSpell: Spell {} (effIndex: {}) has unknown TriggerSpell {}. Unhandled custom case?", auraSpellInfo->Id, triggeredByAura->GetEffIndex(), trigger_spell_id);
        return false;
    }
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, uint32 duration)
{
    ...
    if (!IsNPCBotOrPet() && GetTypeId() == TYPEID_UNIT && enemy && IsImmuneToPC() && enemy->GetCharmerOrOwnerPlayerOrPlayerItself())
...
bool Unit::IsInRaidWith(Unit const* unit) const
{
    ...
    Unit const* u1 = GetCharmerOrOwnerOrSelf();
    Unit const* u2 = unit->GetCharmerOrOwnerOrSelf();
    //Unit const* u2 = unit->IsNPCBot() ? unit->GetOwner() : unit->GetCharmerOrOwnerOrSelf();
