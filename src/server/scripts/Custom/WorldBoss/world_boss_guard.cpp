...
namespace
{
    ...
    std::unordered_map<uint32, WorldBossSpellScale> const BuildWorldBossSpellScaling()
    {
        ...
        // ===================== 奥（Al'ar，风暴要塞复刻） =====================
        map[34121] = { 4.0f, 0.0f }; // 火焰盛宴（近战无目标时的 AOE，DBC 1750-2250）：-> 7000-9000

        for (uint32 spellId = 34269; spellId <= 34289; ++spellId) // 火焰羽刺·羽刺导弹（21 发，DBC 6563-8437）：-> 9844-12655
            map[spellId] = { 1.5f, 0.0f };

        for (uint32 spellId = 34314; spellId <= 34316; ++spellId) // 火焰羽刺·羽刺导弹（3 发，同上）
            map[spellId] = { 1.5f, 0.0f };
        ...
        map[32053] = { 2.0f, 0.0f }; // 灵魂释放·红（灵魂充能释放，DBC 4500 火焰 AOE）：-> 9000
        map[32054] = { 2.0f, 0.0f }; // 灵魂释放·黄（灵魂充能释放，DBC 4500 物理 AOE）：-> 9000
        map[32057] = { 0.0f, 2.0f }; // 灵魂释放·绿（灵魂充能释放，DBC 1125 自然 DOT/跳）：-> 2250/跳

        // 毁灭之火伤害链（施法者为毁灭之火 120516，通过 WorldBossSummonAI 缩放）：
        // 31945（周期触发）-> 31943（区域光环）-> 31944（2400 火焰直伤）。
        // 31945 为入口光环，覆盖触发伤害时施法记录仍停留在 31945 的时序，2 倍 -> 4800/跳。
        map[31945] = { 2.0f, 0.0f };
        ...
        map[31341] = { 0.0f, 2.0f };
        // 注：阿兹加洛的嚎叫（31344）为范围沉默、顺劈斩（31345）为物理近战、末日诅咒（31347）触发末日降临（31348，秒杀），均不缩放。
