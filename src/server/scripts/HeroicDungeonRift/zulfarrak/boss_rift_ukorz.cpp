...
{
    EventRuuzluCleave = 1, // 顺劈斩
    EventRuuzluExecute     // 斩杀
};

enum Spells : uint32
{
    SpellBerserkerStance = 7366, // 狂暴姿态
    SpellCleave = 15496,         // 顺劈斩
    SpellFrenzy = 8269,          // 狂乱（低血量狂暴）
    SpellSunderArmor = 15572,    // 破甲攻击
    SpellThunderclap = 15588,    // 雷霆一击
    SpellExecute = 38959         // 斩杀（卢兹鲁）
};
...
struct npc_rift_ruuzlu : public RiftSummonAI // 裂隙卢兹鲁
{
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        _events.ScheduleEvent(EventRuuzluCleave, Milliseconds(TierDelay(7000, 5500, 4500)));
        _events.ScheduleEvent(EventRuuzluExecute, Milliseconds(TierDelay(10000, 8000, 6500)));
    }
    ...
    void ExecuteAbility(uint32 eventId) override
    {
        switch (eventId)
        {
            case EventRuuzluCleave:
                ...
                _events.ScheduleEvent(EventRuuzluCleave, Milliseconds(TierDelay(9000, 7000, 5500)));
                break;
            case EventRuuzluExecute:
                if (Unit* victim = me->GetVictim())
                    if (victim->HealthBelowPct(20))
                        DoCastVictim(SpellExecute);
                _events.ScheduleEvent(EventRuuzluExecute, Milliseconds(TierDelay(9000, 7500, 6000)));
                break;
            default:
                break;
        }
...
struct boss_rift_ukorz : public BossAIBase
{
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        SummonRuuzlu();
        ScheduleTieredEvent(EventUkorzCleave, 6000, 5000, 4000);
        if (_tier >= 2)
