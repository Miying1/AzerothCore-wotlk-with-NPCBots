        ...
        bool IsAnyPlayerValid()
        {
            ...
            for(Map::PlayerList::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
                if (Player* player = itr->GetSource())
                    if (!player->IsGameMaster() && player->IsAlive() && me->GetHomePosition().GetExactDist2d(player) < 52.0f && me->IsWithinLOSInMap(player) 
                    ...
                    && !player->HasAuraType(SPELL_AURA_FEIGN_DEATH))
                        return true;
            return false;
        }
