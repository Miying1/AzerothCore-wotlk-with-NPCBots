...
struct boss_xt002 : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        // AC core difference: Vehicle::InstallAccessory despawns the heart if HandleSpellClick
        // returns false (TC uses async VehicleJoinEvent and does not check return value).
        // Ensure the heart exists as a fallback.
        if (!instance->GetCreature(DATA_XT002_HEART))
            if (Creature* heart = me->SummonCreature(NPC_HEART_OF_DECONSTRUCTOR, *me, TEMPSUMMON_MANUAL_DESPAWN))
                me->HandleSpellClick(heart, 0);
    }
